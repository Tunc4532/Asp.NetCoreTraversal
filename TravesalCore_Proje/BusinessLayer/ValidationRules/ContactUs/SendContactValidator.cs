...
namespace BusinessLayer.ValidationRules.ContactUs
{
    public class SendContactValidator : AbstractValidator<SendMessageDTO>
    {
        public SendContactValidator()
        {
            ...
            RuleFor(x => x.Subject).MaximumLength(50).WithMessage("Konu alanına en fazla 50 karakter veri girişi yapmalısınız.");
            RuleFor(x => x.Mail).MinimumLength(5).WithMessage("Mail alanına en az 5 karakter veri girişi yapmalısınız.");
            RuleFor(x => x.Mail).MaximumLength(100).WithMessage("Mail alanına en fazla 100 karakter veri girişi yapmalısınız.");
